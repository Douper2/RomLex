...
def getetymology(word):
    ...
    clean_etym = re.sub(r"DEX\s*'?\d+", "", etym_text).strip() # removes the DEX and year text from the etymology
    clean_etym = re.sub(r'\b(DN|DLRM|NODEX)\b', ' ', clean_etym) # removes the internal abbreviations
    return clean_etym
